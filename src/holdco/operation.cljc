(ns holdco.operation
  "OperationActor -- one holding-company operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (HoldCo-LLM) is sealed into a single node (:advise); its proposal is
  ALWAYS routed through the Holding Structure Governor (:govern) and
  ...
  One graph run = one holding-company operation (intake -> advise ->
  ...
  decision to a human operator (a responsible principal). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/disburse-distribution`/`:actuation/record-
  ownership-change` ALWAYS reach this node when the governor is clean
  -- see `holdco.phase`."
...
(defn build
  ...
  `holdco.store/Store`).
  opts:
    :advisor      -- a `holdco.holdcoadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (holdcoadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; HoldCo-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (holdcoadvisor/-advise advisor store request)]
            {:proposal p :audit [(holdcoadvisor/trace request p)]})))

      ;; Holding Structure Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
