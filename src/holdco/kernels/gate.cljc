(ns holdco.kernels.gate
  "Safety kernel for the Holding Structure Governor + phase gate — the
  decision CORE of `holdco.governor/check` and `holdco.phase/gate`,
  ...
  (`holdco.governor`, `holdco.phase`) reduce their inputs to
  ...
  Wire codes:
    ...
    dist        proposed-distribution-amount / distributable-reserves
                in the SAME integer currency units the position record
                itself uses; exceeds iff reserves < amount (the EXACT
                strict-> comparison `holdco.registry/distribution-
                amount-exceeds-distributable-reserves?` makes, on the
                same values, so kernel and façade can never disagree).
                `applicable` is 0 when the op carries no distribution
                check OR either field is missing/non-numeric (matching
                the registry's `number?` guard — the original treats a
                missing field as no-violation, and the façade computes
                that guard).
    op          0 reserved read — this domain's `read-ops` set is EMPTY
                  (holdco.phase), so no façade op ever maps to 0; the
                ...
                1 :position/intake
                2 :disclosure/verify
                3 :beneficial-ownership/screen
                4 :actuation/disburse-distribution
                5 :actuation/record-ownership-change
  ...
  corporate actuations: disbursing a distribution / recording an
  ownership-structure change) are auto-enabled at NO phase — the same
...
(defn distribution-exceeds-reserves
  "1 when the position's own proposed distribution amount STRICTLY
  exceeds its own recorded distributable-reserves ceiling (integer
  units, see ns docstring): exceeds iff reserves < amount. `applicable`
  is 0 when the op carries no distribution check or either field is
  missing (norm-flag, fail-closed: any non-0 applies the check)."
  [applicable amount reserves]
  (if (= (norm-flag applicable) 0)
    0
    (if (< reserves amount) 1 0)))

(defn hard-violation
  "1 when any HARD (human-un-overridable) violation is present:
  spec-basis missing / evidence incomplete / distribution exceeds
  distributable reserves / beneficial-ownership verification
  unresolved / distribution already disbursed / ownership change
  already recorded."
  [spec-missing evidence-incomplete
   dist-applicable amount reserves
   bo-unresolved already-disbursed already-recorded]
  (or3 (or3 (norm-flag spec-missing)
            (norm-flag evidence-incomplete)
            (norm-flag bo-unresolved))
       (or2 (norm-flag already-disbursed)
            (norm-flag already-recorded))
       (distribution-exceeds-reserves dist-applicable amount reserves)))
...
(defn verdict-code
  ...
  (if (= 1 (hard-violation spec-missing evidence-incomplete
                           dist-applicable amount reserves
                           bo-unresolved already-disbursed already-recorded))
    2
...
(defn op-auto-enabled
  ...
  column). Exactly one cell is ever 1: phase 3 x :position/intake.
  Ops 4 and 5 (:actuation/disburse-distribution /
  :actuation/record-ownership-change) are 0 at every phase — permanent
  structural fact, not a rollout milestone. Op 3
  (:beneficial-ownership/screen) is likewise 0 at every phase."
  [phase op]
  (if (= phase 3) (if (= op 1) 1 0) 0))

(defn phase-disposition
  "Resolve the final disposition code from phase, op code and the
  governor's disposition code. Mirrors `holdco.phase/gate`: governor
...
(defn check-verdict [spec evid dapp amt rsv bo disb rec conf act expected]
  (if (= (verdict-code spec evid dapp amt rsv bo disb rec conf act) expected)
    1 0))

(defn check-dist [applicable amount reserves expected]
  (if (= (distribution-exceeds-reserves applicable amount reserves) expected) 1 0))
...
(defn battery-pass-count []
  (+
   ...
   (check-verdict 0 0 1 3000000 2000000 0 0 0 100 0 2)
   ...
   (check-verdict 1 0 0 0 0 1 0 0 100 0 2)
   (check-verdict 0 0 1 3000000 2000000 0 1 0 100 0 2)
   (check-verdict 1 1 1 3000000 2000000 1 1 1 100 0 2)
   ...
   (check-verdict 0 0 0 0 0 5 0 0 100 0 2)
   (check-verdict 0 0 0 0 0 0 0 0 100 9 1)
   ;; -- distribution: strict-> boundary (exceeds iff reserves < amount)
   (check-dist 1 500 500 0)
   (check-dist 1 501 500 1)
   (check-dist 1 499 500 0)
   ;; -- distribution: not applicable / fail-closed applicable flag
   (check-dist 0 501 500 0)
   (check-dist 7 501 500 1)
