(ns holdco.portable-cljs-test-runner
  ...
  `holdco.store-contract-test`, which exercises the langchain.db
  ...
  -m if combined, hence -Sdeps for the extra path):
    ...
    clojure -Sdeps '{:paths [\"src\" \"test\"]}' \\
      ...
      --output-to target/tests.cjs -c holdco.portable-cljs-test-runner
  ...
  (:require [clojure.test :as t :refer [run-tests]]
            [holdco.corporate-intel-test]
            [holdco.facts-test]
            [holdco.governor-contract-test]
            [holdco.kernels.gate-test]
            [holdco.phase-test]
            [holdco.registry-test]
            [holdco.store-contract-test]))
